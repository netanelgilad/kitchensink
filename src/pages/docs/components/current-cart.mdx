...
- `selectedOptions: Array<{ name: string; value: string | { text: string; color: string } }>` - Selected product options (e.g., size, color)
- `hasSelectedOptions: boolean` - Whether this item has selected options
            ...
            <div className="flex flex-wrap gap-2">
              {selectedOptions.map((option, index) => {
                ...
                const displayText = isColorOption
                  ? (option.value as { text: string; color: string }).text
                  : (option.value as string);
                const colorCode = isColorOption
                  ? (option.value as { text: string; color: string }).color
                  : undefined;
