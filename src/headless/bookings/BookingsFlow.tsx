...
const AvailabilitySlots = (props: AvailabilitySlotsProps) => {
  ...
  const state = bookingsService.bookingState.get();
  const isLoading = state.status === BookingStatusEnum.LOADING_AVAILABILITY;
 ...
 */
const BookingStatusComponent = (props: BookingStatusProps) => {
  const bookingsService = useService(
    BookingsServiceDefinition
  ) as ServiceAPI<typeof BookingsServiceDefinition>;
  ...
  if (bookingState.status === BookingStatusEnum.ERROR || checkoutState.status === CheckoutStatusEnum.ERROR) {
    currentStep = 'error';
...
export const BookingsFlow = {
  ...
  AvailabilitySlots,
  BookingCheckout,
  BookingStatus: BookingStatusComponent,
}; 